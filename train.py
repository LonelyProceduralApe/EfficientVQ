...
from utils.operate import AverageMeter, resize
...
def get_canvas(
    ...
    mask: np.ndarray,         # 掩码图像，格式为numpy数组
    colors: tuple or list,    # 颜色列表，用于不同类别的掩码
    opacity=0.5,              # 掩码的不透明度
) -> np.ndarray:
    # 获取图像和掩码的尺寸
    image_shape = image.shape[:2] # 图像的高度和宽度
    mask_shape = mask.shape       # 掩码的尺寸

    # 如果图像和掩码尺寸不一致，则调整掩码的尺寸以匹配图像
    if image_shape != mask_shape:
        mask = cv2.resize(mask, dsize=(image_shape[1], image_shape[0]), interpolation=cv2.INTER_NEAREST)

    # 创建一个与原图像大小相同的全零数组作为分割掩码
    seg_mask = np.zeros_like(image, dtype=np.uint8)

    # 遍历所有颜色，并将对应类别的掩码区域染色
    for k, color in enumerate(colors):
        seg_mask[mask == k, :] = color

    # 将着色后的掩码与原图像结合，根据不透明度混合
...
def create_seg_model(
    dataset: str, pretrained=False, weight_url: str or None = None, **kwargs
) -> EfficientVQ:
    model = efficientVQ(dataset=dataset, **kwargs)
...
def main():
    ...
    parser.add_argument("--path", type=str, default="./dataset/cityscapes/leftImg8bit/train/", help="path to dataset")
    ...
    # 加载模型
    model = create_seg_model(args.dataset, weight_url=args.weight_url)
    ...
    for epoch in range(args.num_epochs):
        with tqdm(total=len(data_loader), desc=f"Eval model on {args.dataset}") as t:
            for feed_dict in data_loader:
                ...
                loss = criterion(output, mask)  # 计算损失
                ...
                current_mIoU = (interaction.sum / union.sum).cpu().mean().item()

                # 更新进度条
                t.set_postfix(
                    {
                        "mIOU": current_mIoU * 100,
                        "loss": loss.item(),
                    }
                )
                t.update()
            ...
            # 保存当前模型
            current_model_path = os.path.join(args.save_path, "current.pth")
            torch.save(model.state_dict(), os.path.join(args.save_path, current_model_path))

            if current_mIoU > best_mIoU:
                best_mIoU = current_mIoU
                # 保存最佳模型
                best_model_path = os.path.join(args.save_path, "best.pth")
                torch.save(model.state_dict(), os.path.join(args.save_path, best_model_path))
                print(f"Epoch {epoch}: New best model saved with mIoU: {best_mIoU:.3f}")
            else:
                print(f"Epoch {epoch}: Current mIoU: {current_mIoU:.3f}, Best mIoU: {best_mIoU:.3f}")
